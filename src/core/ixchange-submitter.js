import { logger } from '../utils/logger.js';
import { PORTALS } from '../config/portals.js';

/**
 * Dedicated submit service boundary for IXCHANGE portal flow.
 */
export class IXChangeSubmitter {
  constructor(steps = null) {
    this.steps = steps;
  }

  async submit(visit, runtimeCredential = null) {
    if (this.steps?.step) {
      this.steps.step(2, 'Submitting to IXCHANGE portal service');
    }

    logger.warn('[SUBMIT] IXCHANGE portal automation not yet implemented', {
      visitId: visit?.id || null,
      payType: visit?.pay_type || null,
    });

    return {
      success: false,
      reason: 'not_implemented',
      error: 'IXCHANGE portal submit service is selected but automation is not implemented yet',
      portal: 'IXCHANGE',
      portalService: 'IXCHANGE',
      detailReason: 'ixchange_portal_automation_not_implemented',
      portalUrl: runtimeCredential?.url || PORTALS.IXCHANGE?.url || null,
      hasRuntimeCredential: Boolean(runtimeCredential?.username || runtimeCredential?.password),
      savedAsDraft: false,
      submitted: false,
    };
  }
}

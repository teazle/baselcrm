...
/**
 * Test script to verify access to mhcasia.net with Singapore VPN
 * This script will:
 * 1. Launch browser with Urban VPN extension (if configured)
 * 2. Navigate to mhcasia.net
 * 3. Check IP location
 * 4. Test if the site is accessible
 */
...
async function checkIPLocation(page) {
  try {
    ...
    if (jsonMatch) {
      try {
        ...
        if (ipInfo.country === 'SG') {
          logger.info('✓ IP is from Singapore - VPN is working!');
          return true;
        } else {
          logger.warn(`⚠ IP is from ${ipInfo.country || 'Unknown'} - VPN may not be connected to Singapore`);
          return false;
        }
      } catch (e) {
    ...
    // Fallback: try another IP check service
    ...
    if (ipText) {
      ...
      logger.info(`Current IP: ${ipData.ip}`);
    }
...
async function testMHCAsiaAccess() {
  ...
  try {
    logger.info('=== Testing MHC Asia Access with Singapore VPN ===\n');
    ...
    if (!isSingapore) {
      ...
      logger.warn('If using Urban VPN extension, please:');
      logger.warn('1. Check that the extension is enabled');
      logger.warn('2. Connect to Singapore server in the extension');
      logger.warn('3. Wait a few seconds for connection to establish');
      logger.warn('\nContinuing with access test anyway...\n');
    }
    ...
    try {
      ...
      // Take a screenshot
      const screenshotPath = 'screenshots/mhcasia-access-test.png';
      ...
      logger.info('\n=== Test Complete ===');
      logger.info('If you can see the login page, the VPN is working correctly!');
      logger.info('You can now use the automation with Singapore VPN enabled.\n');

    } catch (error) {
      logger.error('✗ Failed to access mhcasia.net:', error.message);
      logger.error('\nPossible issues:');
      logger.error('1. VPN not connected to Singapore');
      logger.error('2. Site is blocking automated browsers');
      logger.error('3. Network connectivity issues');
      logger.error('4. Extension not properly configured');
      
      // Take error screenshot
      try {
        await page.screenshot({ path: 'screenshots/mhcasia-access-error.png', fullPage: true });
        logger.info('Error screenshot saved: screenshots/mhcasia-access-error.png');
      } catch (e) {

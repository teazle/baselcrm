...
### 6. EC2_ENV_FILE (Recommended)

**Secret Name**: `EC2_ENV_FILE`  
**Value**: Full content of your local root `.env` file.  
**Use case**: Deploy workflow will write this to `~/Baselrpacrm/.env` on EC2 before restarting services.

**Set from local machine**:

```bash
gh secret set EC2_ENV_FILE < .env
```

---

### 7. EC2_CRM_ENV_LOCAL (Optional, recommended for CRM)

**Secret Name**: `EC2_CRM_ENV_LOCAL`  
**Value**: Full content of your local `apps/crm/.env.local` file.  
**Use case**: Deploy workflow writes this to `apps/crm/.env.local` on EC2.  
**Fallback**: If not set, workflow copies `EC2_ENV_FILE` into `apps/crm/.env.local`.

**Set from local machine**:

```bash
gh secret set EC2_CRM_ENV_LOCAL < apps/crm/.env.local
```
...
4. Click **"Add secret"**
5. Repeat for all required secrets, then add env file secrets if you want automatic env sync

...
export class MHCAsiaAutomation {
  ...
  async navigateToAIAProgramSearch() {
    try {
      ...
      logger.info('Navigating to AIA Program search...');
      
      // Try searchAIA=N first (primary)
      const baseUrl = this.config.url.replace(/\/$/, ''); // Remove trailing slash
      const searchUrlN = `${baseUrl}/ClinicEmpVisitSelectPatient.ec?searchAIA=N`;
      
      this._logStep('Trying searchAIA=N (primary)');
      await this.page.goto(searchUrlN, { waitUntil: 'domcontentloaded', timeout: 30000 });
      await this.page.waitForTimeout(2000);
      
      // Check for error message that says to use AIA
      const errorSelectors = [
        'text=/use.*AIA/i',
        'text=/AIA.*member/i',
        '.error:has-text("AIA")',
      ...
      for (const selector of errorSelectors) {
        try {
          ...
          if ((await errorElement.count().catch(() => 0)) > 0) {
            const errorText = await errorElement.textContent();
            if (errorText && errorText.toLowerCase().includes('aia')) {
              needsAIA = true;
              this._logStep('Error detected - need to use searchAIA=Y', { error: errorText.substring(0, 100) });
              break;
            }
          }
        } catch (e) {
      ...
      // Fallback to searchAIA=Y if error detected
      if (needsAIA) {
        const searchUrlY = `${baseUrl}/ClinicEmpVisitSelectPatient.ec?searchAIA=Y`;
        this._logStep('Falling back to searchAIA=Y');
        logger.info('Switching to searchAIA=Y due to error');
        
        await this.page.goto(searchUrlY, { waitUntil: 'domcontentloaded', timeout: 30000 });
        await this.page.waitForTimeout(2000);
      }
  ...
  async fillConsultationFee(fee) {
    try {
      ...
      // Try to find the consultation fee field using various methods
      const feeSelectors = [
      ...
      for (const selector of feeSelectors) {
        try {
          ...
          if ((await field.count().catch(() => 0)) > 0) {
            await field.clear();
            await field.fill(fee.toString());
            this._logStep('Consultation fee filled via selector', { fee, selector });
            return true;
          }
        } catch (e) {
          continue;
      ...
      try {
        const feeLabel = this.page.locator('td:has-text("Consultation Fee"), label:has-text("Consultation Fee")').first();
        if (await feeLabel.count() > 0) {
          ...
          if (await input.count() > 0) {
            await input.clear();
            await input.fill(fee.toString());
            this._logStep('Consultation fee filled via label', { fee });
            return true;
          }
        }
      } catch (e) {
      ...
      // Try JavaScript approach to find and fill the field
      try {
        const filled = await this.page.evaluate((feeValue) => {
          // Look for input fields with consultation fee label nearby
          ...
          // Also try finding by name containing "consult"
          const consultInput = document.querySelector('input[name*="onsult"]');
          if (consultInput) {
            consultInput.value = feeValue;
            consultInput.dispatchEvent(new Event('change', { bubbles: true }));
            return true;
          }
          return false;
        }, fee.toString());
        ...
        if (filled) {
          this._logStep('Consultation fee filled via JavaScript', { fee });
          return true;
        }
      } catch (e) {
        // Continue
      ...
      logger.warn('Consultation fee field not found');
      return false;
    } catch (error) {
  ...
  async fillDiagnosisPrimary(diagnosisText) {
    try {
      ...
      // Find and click "M" button for Diagnosis Pri
      const mButtonSelectors = [
        'button:has-text("M"):near(select[name*="diagnosis" i], 100)',
        'button:has-text("M"):near(text="Diagnosis Pri", 100)',
        'td:has-text("Diagnosis Pri") button:has-text("M")',
        'input[name*="diagnosis" i] + button:has-text("M")',
      ];
      ...
      for (const selector of mButtonSelectors) {
        try {
          ...
          if ((await mButton.count().catch(() => 0)) > 0) {
            await mButton.click();
            this._logStep('Clicked M button for diagnosis search');
            mButtonFound = true;
            break;
          }
        } catch (e) {

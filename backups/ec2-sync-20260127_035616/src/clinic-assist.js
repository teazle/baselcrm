...
export class ClinicAssistAutomation {
  ...
  async navigateToPatientPage() {
    try {
      ...
      // Quick DOM ready check - no need to wait for network idle
      await this.page.waitForLoadState('domcontentloaded');
        ...
        for (const selector of roomSelectors) {
          try {
            ...
            if (count > 0) {
              ...
              if (isVisible) {
                this._logStep('Clicking Reception/room to access system', { selector });
                // Use force click and JavaScript fallback to avoid dialog blocking
                try {
                  await roomLink.click({ timeout: 3000, force: true });
                } catch (e) {
                  this._logStep('Force click failed, using JavaScript click', { error: e.message });
                  await roomLink.evaluate(el => el.click());
                }
                await this.page.waitForLoadState('domcontentloaded', { timeout: 5000 }).catch(() => {});
                // Wait for navigation to room page (e.g. /Home/Reception)
                const urlChanged = await this.page.waitForURL(/\/Home\//, { timeout: 5000 }).catch(() => false);
  ...
  async searchPatientByNumber(patientNumber) {
    try {
      ...
      await this.page.keyboard.press('Enter');
      await this.page.waitForTimeout(2000);
      this._logStep('Patient search executed by number (Enter key)', { patientNumber });
      return true;
    } catch (error) {

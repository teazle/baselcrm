...
function reconcileReceiptAndVisitDemo(input: { receiptId?: string | null; visitId?: string | null }) {
  const receiptId = input.receiptId ?? null;
  const visitId = input.visitId ?? null;
...
export async function sbList<T>(table: string, opts?: { select?: string; order?: { column: string; ascending?: boolean }; limit?: number }) {
  const supabase = supabaseBrowser();
...
export async function sbInsert<T>(table: string, values: Record<string, unknown>, select = "*"): Promise<SbResult<T>> {
  const supabase = supabaseBrowser();
  if (isDemoMode() || !supabase) {
    ...
    if (table === "visits") {
      if (!base.visit_record_no) base.visit_record_no = nextAutoNo("visits", "visit_record_no", "V-");
      if (!base.patient_name) {
  ...
  if (table === "receipt_visit_offsets") {
    await reconcileReceiptAndVisitSupabase({
      ...
      visitId: typeof (values as any)?.visit_id === "string" ? String((values as any).visit_id) : null,
    });
  }
  return { data: data as unknown as T, error: null };
}

export async function sbUpdate<T>(table: string, id: string, values: Record<string, unknown>, select = "*"): Promise<SbResult<T>> {
  const supabase = supabaseBrowser();
  if (isDemoMode() || !supabase) {
    const rows = mockGetTable(table);
    ...
    if (table === "receipt_visit_offsets") {
      const receiptId =
        ...
        (typeof (existing as any)?.receipt_id === "string" && String((existing as any).receipt_id)) ||
        null;
      const visitId =
  ...
  const { data, error } = await supabase.from(table).update(values).eq("id", id).select(select).single();
  if (error) {
    if (mockFallbackTables.has(table) && isMissingTableError(error)) {
      const rows = mockGetTable(table);
      const existing = rows.find((r) => String(r.id) === id);
...
export async function sbDelete(table: string, id: string): Promise<SbResult<true>> {
  ...
  if (isDemoMode() || !supabase) {
    ...
    if (table === "receipt_visit_offsets" && existing) {
      reconcileReceiptAndVisitDemo({
        receiptId: typeof (existing as any)?.receipt_id === "string" ? String((existing as any).receipt_id) : null,
        visitId: typeof (existing as any)?.visit_id === "string" ? String((existing as any).visit_id) : null,
      });
    }

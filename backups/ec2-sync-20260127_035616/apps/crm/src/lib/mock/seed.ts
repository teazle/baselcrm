...
import { mockGetTable, mockSetTable } from "@/lib/mock/storage";

function ensure(table: string, rows: Array<Record<string, unknown>>) {
  const existing = mockGetTable(table);
  if (existing.length > 0) return;
  mockSetTable(table, rows as unknown as Array<Record<string, unknown>>);
}

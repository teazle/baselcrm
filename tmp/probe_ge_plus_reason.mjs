...
const state=async p=>p.evaluate(()=>({
 ...
 mcText:document.querySelector('#ctl00_MainContent_uc_MakeClaim_txtmcreasons')?.value||'',
...
try{
 await auto.login(); await auto.navigateToMedicalTreatmentClaim(); const found=await auto.searchMemberByNric(nric,date); if(!found?.found) throw new Error('not found');
 ...
 await p.click('#ctl00_MainContent_uc_MakeClaim_btnmcreasons').catch(()=>{});
 ...
 let code='R52';
 if(row){ code=await row.$eval('td:nth-child(1)',el=>(el.textContent||'').trim()).catch(()=>code); const link=await row.$('a[href*="lbtnPrimaryDiagnosis"]'); if(link) await link.click().catch(async()=>link.click({force:true})); }
 await p.waitForTimeout(1200);
 await p.evaluate((v)=>{ const c=document.querySelector('#ctl00_MainContent_uc_MakeClaim_txtprimarydiagnosisCode'); const h=document.querySelector('#ctl00_MainContent_uc_MakeClaim_hfPrimaryDiagnosisCode'); if(c){c.value=v;c.dispatchEvent(new Event('input',{bubbles:true}));c.dispatchEvent(new Event('change',{bubbles:true}));} if(h) h.value=v; }, code);
 await p.waitForTimeout(300);
 console.log('[probe] after diag', JSON.stringify(await state(p),null,2));
 ...
 await p.screenshot({path:'/Users/vincent/Baselrpacrm/screenshots/ge-popup-plus-reason.png',fullPage:true}).catch(()=>{});

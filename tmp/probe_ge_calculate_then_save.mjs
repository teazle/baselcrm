...
const state=async p=>p.evaluate(()=>({
 ...
 calc:!!document.querySelector('#ctl00_MainContent_uc_MakeClaim_btncalculateclaim'),
 buttons:Array.from(document.querySelectorAll('input[type="submit"],input[type="button"],button')).map(el=>({id:el.id||'',name:el.name||'',v:el.value||'',vis:!!(el.offsetParent||el.getClientRects().length)})),
 totals:{
 total:document.querySelector('#ctl00_MainContent_uc_MakeClaim_lblTotalAfterGST')?.textContent?.trim()||'',
 payable:document.querySelector('#ctl00_MainContent_uc_MakeClaim_lblAmountPayable')?.textContent?.trim()||'',
 }
}));
...
try{
 ...
 console.log('[probe] before calculate', JSON.stringify(await state(p),null,2));

 const calc = p.locator('#ctl00_MainContent_uc_MakeClaim_btncalculateclaim, input[value*="Calculate Claim" i], button:has-text("Calculate Claim")').first();
 ...
 console.log('[probe] after calculate', JSON.stringify(await state(p),null,2));
 await p.screenshot({path:'/Users/vincent/Baselrpacrm/screenshots/ge-popup-after-calculate-probe.png',fullPage:true}).catch(()=>{});

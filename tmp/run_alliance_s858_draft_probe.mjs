...
const visit = {
  ...
  nric: 'S8580559B',
  diagnosis_description: 'Cervicalgia',
  treatment_detail: 'Medication and review',
  total_amount: 38.0,
  extraction_metadata: {
    chargeType: 'follow',
    diagnosisCode: 'M54.2',
...
try {
  ...
  if (!search?.found) {
    await page.screenshot({ path: 'screenshots/alliance-s858-not-found.png', fullPage: true }).catch(() => {});
    process.exitCode = 2;
  } else {
    await auto.selectMemberAndAdd();
    await auto.fillClaimForm(visit, 'Palanisamy Arul Murugan');
    const saved = await auto.saveAsDraft();
    await page.screenshot({ path: 'screenshots/alliance-s858-after-save-attempt.png', fullPage: true }).catch(() => {});
  ...
  await page.screenshot({ path: 'screenshots/alliance-s858-probe-error.png', fullPage: true }).catch(() => {});
  process.exitCode = 1;
} finally {
  await bm.close();
}

...
export const FULLERTON_TAGS = ['FULLERT', 'AONCARE'] as const;
export const IHP_TAGS = ['IHP'] as const;
export const IXCHANGE_TAGS = ['ALL', 'ALL_PW', 'PARKWAY'] as const;
export const GE_NTUC_TAGS = ['GE'] as const;
export const MHC_TAGS = ['MHC', 'AVIVA', 'NTUC_IM', 'MHCAXA'] as const;
...
export const CA_PORTAL_TAG_ROUTES = Object.freeze({
  ...
  GE_NTUC: ['GE'],
} satisfies Record<Flow3PortalTarget, readonly string[]>);
...
export type ClaimCandidateClassification = {
  ...
  portalRouting?: Flow3PortalRouting;
  normalizedPayType: string;
  hasNric: boolean;
};

export type Flow3PortalRouting = {
  portalTarget: Flow3PortalTarget | null;
  source: string | null;
  tag: string | null;
  reason: string;
...
function hasFilledUnverifiedFields(fillVerification: unknown): boolean {
  const fv =
    fillVerification && typeof fillVerification === 'object'
      ? (fillVerification as Record<string, unknown>)
      : null;
  if (!fv) return false;
...
function normalizedAmbiguousInsurerName(value: string | null | undefined): string | null {
...
function findMatchedPortalTag(
  payType: string | null | undefined,
  patientName: string | null | undefined,
  tags: readonly string[],
  { includePatientName = true }: { includePatientName?: boolean } = {}
...
export function resolveFlow3PortalRouting(
  payType: string | null | undefined,
  patientName: string | null | undefined,
  extractionMetadata: unknown = null
): Flow3PortalRouting {
  for (const [portalTarget, tags] of Object.entries(CA_PORTAL_TAG_ROUTES) as Array<
    [Flow3PortalTarget, readonly string[]]
  >) {
  ...
  const legacyChecks: Array<[Flow3PortalTarget, readonly string[], boolean]> = [
    ['MHC', MHC_TAGS, false],
...
export function describePortalRouting(
  payType: string | null | undefined,
  patientName: string | null | undefined,
  extractionMetadata: unknown = null
) {
